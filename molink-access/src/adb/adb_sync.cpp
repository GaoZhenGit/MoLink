...
bool syncStat(AdbClient& client, ChannelPtr ch,
    ...
    if (!sendSyncMsg(client, ch, SYNC_STAT,
                     remotePath.data(), (uint32_t)remotePath.size())) {
        LOG_ERROR("SYNC", "Failed to send STAT");
        return false;
    }
    ...
    if (resp.id == SYNC_FAIL) {
        std::string err(payload.begin(), payload.end());
        LOG_ERROR("SYNC", "STAT failed: %s", err.c_str());
        return false;
    }
    if (resp.id != SYNC_STAT) {
        LOG_ERROR("SYNC", "STAT unexpected response id 0x%08X", resp.id);
        return false;
    }

    // Payload: "<octal-mode> <size> <mtime>\0" (adbd 不发末尾 \0，留余量给 sscanf)
    std::string s(payload.begin(), payload.end());
    int mode_i = 0;
    long long size_i = 0;
    unsigned int mtime_i = 0;
    if (sscanf(s.c_str(), "%o %lld %u", &mode_i, &size_i, &mtime_i) != 3) {
        LOG_ERROR("SYNC", "STAT parse error: %s", s.c_str());
        return false;
    }
    if (mode)  *mode  = (uint32_t)mode_i;
    if (size)  *size  = (uint32_t)size_i;
    if (mtime) *mtime = mtime_i;
    return true;
}